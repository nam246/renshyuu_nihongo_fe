import { examples, kanjis } from "@/lib/mock-data";
...
export default async function KanjiDetails({
  params,
}: {
  ...
  const kanjisById = kanjis.find((v) => v.id === id);
      ...
      <Card>
        ...
        <div className="bg-primary p-6 text-white">
          <div className="space-y-3">
            <div className="flex items-start justify-between">
              <div className="space-y-2 flex-1">
                <h1 className="text-4xl font-bold tracking-tight">
                  {kanjisById.character}
                </h1>
                <p className="text-xl text-blue-50 font-light">
                  Onyomi: {kanjisById.onyomi}
                </p>
                <p className="text-xl text-blue-50 font-light">
                  Kunyomi: {kanjisById.kunyomi}
                </p>
              </div>
            ...
            <Badge variant="secondary">{kanjisById.strokeCount}</Badge>
          </div>
        </div>
        ...
        <CardHeader className="border-b bg-slate-50">
          <CardTitle className="text-lg font-semibold text-slate-700 flex items-center gap-2">
            <BookOpen className="w-5 h-5" />
            Ví dụ câu ({kanjisById.exampleId?.length})
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="p-6">
          {kanjisById.exampleId?.length === 0 ? (
            ...
            <div className="space-y-4">
              {kanjisById.exampleId?.map((exId, index) => {
                const example = getExampleById(exId);

                if (!example) return null;

                return (
                  <Item key={exId} variant="outline">
                    <ItemHeader></ItemHeader>
                    <ItemContent className="p-4">
                      <div className="flex items-start gap-3">
                        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-primary flex items-center justify-center text-white font-semibold text-sm">
                          {index + 1}
                        </div>
